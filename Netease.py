...
def make_dir(key_p, key_c):
    ...
    path = './Netease/' + key_p + '/' + key_c
    if not os.path.exists(path):
        os.mkdir(path)

    path = './Netease/' + key_p + '/' + key_c + '/' + format_time()
    if not os.path.exists(path):
        os.mkdir(path)
...
def urls_datas(list_nums):
    TEMP = {
            'internal': {
                'index': 'https://temp.163.com/special/00804KVA/cm_guonei.js?callback=data_callback',
                'post': f'https://temp.163.com/special/00804KVA/cm_guonei_{list_nums}.js?callback=data_callback'},
    }
